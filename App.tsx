...
const COLORS = {
  High: '#c45c3a',      // Warm red-brown
  Medium: '#d4a24c',    // Warm amber
  Low: '#8b7355',       // Warm taupe
  Completed: '#6b8b5a', // Sage green
  Pending: '#a67c52',   // Mocha brown (primary)
};
...
const Modal = ({ isOpen, onClose, title, children }: { isOpen: boolean; onClose: () => void; title: string; children?: React.ReactNode }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fadeIn">
      <div className="bg-warm-50 dark:bg-warm-800 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden transform transition-all scale-100 flex flex-col max-h-[90vh]">
        <div className="px-6 py-4 border-b border-warm-200 dark:border-warm-700 flex justify-between items-center shrink-0">
          <h3 className="text-xl font-bold text-warm-900 dark:text-warm-50">{title}</h3>
          <button onClick={onClose} className="text-warm-500 hover:text-warm-700 dark:text-warm-400 dark:hover:text-gray-200">
            <X size={20} />
          </button>
        </div>
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[60] flex flex-col gap-3 pointer-events-none">
      {toasts.map((toast) => (
        <div
          key={toast.id}
          className={`pointer-events-auto flex items-center gap-3 px-4 py-3 rounded-lg shadow-lg text-white transform transition-all animate-slideIn ${toast.type === 'success' ? 'bg-emerald-600' :
...
const TaskCard: React.FC<{
  ...
  if (viewMode === 'list') {
    return (
      <div className={`group bg-warm-50 dark:bg-warm-800 rounded-lg border border-warm-200 dark:border-warm-700 hover:shadow-md transition-all flex items-center p-3 gap-4 ${task.status === 'Completed' ? 'opacity-75' : ''}`}>
        {/* Status Checkbox */}
        {!isRecycleBin && (
          <button
            onClick={() => onToggleStatus(task.id)}
            className={`flex-shrink-0 w-5 h-5 rounded border transition-colors flex items-center justify-center ${task.status === 'Completed'
              ? 'bg-emerald-500 border-emerald-500 text-white'
              : 'border-warm-300 dark:border-slate-500 hover:border-primary-500'
              }`}
          >
        ...
        <div className="flex-1 min-w-0 grid grid-cols-1 md:grid-cols-4 gap-2 items-center">
          <div className="md:col-span-2">
            <h3 className={`font-semibold text-sm text-warm-900 dark:text-warm-50 truncate ${task.status === 'Completed' ? 'line-through text-warm-500' : ''}`}>
              {task.title}
            </h3>
            <p className="text-xs text-warm-500 dark:text-warm-400 truncate">{task.description}</p>
          </div>

          <div className="flex items-center gap-2 text-xs">
            <span className={`px-2 py-0.5 rounded-full ${task.priority === 'High' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300' :
              task.priority === 'Medium' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300' :
                'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300'
              }`}>
              {task.priority}
            </span>
            <div className="flex items-center gap-1 text-warm-500 dark:text-warm-400">
          ...
          <div className="text-xs text-warm-500 dark:text-warm-400 hidden md:block">
        ...
        <div className="relative flex-shrink-0">
          <button
            ...
            className="text-warm-400 hover:text-warm-600 dark:hover:text-gray-200 p-1"
          >
            <MoreVertical size={16} />
          </button>
          <div id={`menu-list-${task.id}`} className="hidden absolute right-0 top-8 w-40 bg-warm-50 dark:bg-warm-700 rounded-lg shadow-xl border border-warm-100 dark:border-warm-600 z-50 animate-fadeIn" onMouseLeave={(e) => e.currentTarget.classList.add('hidden')}>
            {isRecycleBin ? (
              <>
              ...
              <>
                <button onClick={(e) => { e.stopPropagation(); onEdit(task); }} className="w-full text-left px-4 py-2 text-sm text-warm-700 dark:text-warm-200 hover:bg-warm-50 dark:hover:bg-warm-600 flex items-center gap-2">
                  <Edit2 size={14} /> Edit
                </button>
                <button onClick={(e) => { e.stopPropagation(); onArchive && onArchive(task); }} className="w-full text-left px-4 py-2 text-sm text-warm-700 dark:text-warm-200 hover:bg-warm-50 dark:hover:bg-warm-600 flex items-center gap-2">
  ...
  return (
    <div
      className={`group bg-warm-50 dark:bg-warm-800 rounded-xl border transition-all hover:shadow-lg flex flex-col h-auto ${task.status === 'Completed'
        ? 'border-warm-200 dark:border-warm-700 opacity-75'
        : 'border-warm-200 dark:border-warm-700 hover:border-primary-300 dark:hover:border-primary-700'
        }`}
    >
      {task.imageUrl && (
        <div className="h-32 w-full overflow-hidden rounded-t-xl bg-warm-100 relative shrink-0">
      ...
      <div className="p-5 flex-1 flex flex-col">
        <div className="flex justify-between items-start mb-3">
          <div className={`px-2.5 py-1 rounded-full text-xs font-semibold ${task.priority === 'High' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300' :
            task.priority === 'Medium' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300' :
              'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300'
            }`}>
            {task.priority}
          ...
          <div className="relative">
            <button
              ...
              className="text-warm-400 hover:text-warm-600 dark:hover:text-gray-200 p-1 rounded-full hover:bg-warm-100 dark:hover:bg-warm-700"
            >
              <MoreVertical size={18} />
            </button>
            <div id={`menu-${task.id}`} className="hidden absolute right-0 mt-2 w-40 bg-warm-50 dark:bg-warm-700 rounded-lg shadow-xl border border-warm-100 dark:border-warm-600 z-50 animate-fadeIn" onMouseLeave={(e) => e.currentTarget.classList.add('hidden')}>
              {isRecycleBin ? (
                <>
                ...
                <>
                  <button onClick={(e) => { e.stopPropagation(); onEdit(task); document.getElementById(`menu-${task.id}`)?.classList.add('hidden'); }} className="w-full text-left px-4 py-2 text-sm text-warm-700 dark:text-warm-200 hover:bg-warm-50 dark:hover:bg-warm-600 flex items-center gap-2">
                    <Edit2 size={14} /> Edit
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); onArchive && onArchive(task); document.getElementById(`menu-${task.id}`)?.classList.add('hidden'); }} className="w-full text-left px-4 py-2 text-sm text-warm-700 dark:text-warm-200 hover:bg-warm-50 dark:hover:bg-warm-600 flex items-center gap-2">
                    <Archive size={14} /> {task.isArchived ? 'Unarchive' : 'Archive'}
                  </button>
        ...
        <div className="flex items-start gap-3 mb-3 flex-1">
          {!isRecycleBin && (
            <button
              onClick={() => onToggleStatus(task.id)}
              className={`mt-1 flex-shrink-0 w-5 h-5 rounded border transition-colors flex items-center justify-center ${task.status === 'Completed'
                ? 'bg-emerald-500 border-emerald-500 text-white'
                : 'border-warm-300 dark:border-slate-500 hover:border-primary-500'
                }`}
            >
          ...
          <div className="w-full min-w-0">
            <h3 className={`font-semibold text-warm-900 dark:text-warm-50 mb-1 truncate ${task.status === 'Completed' ? 'line-through text-warm-500' : ''}`}>
              {task.title}
            </h3>
            <p className="text-sm text-warm-500 dark:text-warm-400 line-clamp-2">
              {task.description}
            </p>
            ...
            {task.subtasks.length > 0 && (
              <div className="mt-3">
                ...
                <div className="w-full bg-warm-200 dark:bg-warm-700 rounded-full h-1.5 overflow-hidden mb-2">
                  <div className="bg-primary-500 h-1.5 rounded-full transition-all duration-300" style={{ width: `${progress}%` }}></div>
                </div>
                {!isRecycleBin && (
                  <div className="space-y-1 max-h-32 overflow-y-auto custom-scrollbar">
                    {task.subtasks.map(st => (
                      <div key={st.id} onClick={() => onToggleSubtask(task.id, st.id)} className="flex items-start gap-2 text-xs text-warm-600 dark:text-warm-400 cursor-pointer hover:text-warm-900 dark:hover:text-white group/subtask">
                        <div className={`mt-0.5 w-3 h-3 rounded border flex items-center justify-center flex-shrink-0 ${st.completed ? 'bg-primary-500 border-primary-500' : 'border-warm-400 group-hover/subtask:border-primary-500'}`}>
        ...
        <div className="flex items-center justify-between mt-4 pt-4 border-t border-warm-100 dark:border-warm-700">
          <div className="flex items-center gap-2 text-xs text-warm-500 dark:text-warm-400">
            <CalendarIcon size={14} />
            <span className={`${new Date(task.deadline).getTime() < Date.now() && task.status !== 'Completed' ? 'text-red-500 font-bold' : ''
              }`}>
              {formatDate(task.deadline)}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-xs px-2 py-1 bg-warm-100 dark:bg-warm-700 rounded text-warm-600 dark:text-warm-300">
              {task.category}
            </span>
          </div>
        </div>
...
// Dashboard Application Component (protected)
const DashboardApp: React.FC<{
...
}> = ({ user, setUser, darkMode, setDarkMode, toasts, addToast, removeToast }) => {
  ...
  const currentView = useMemo(() => {
    ...
    if (path === '/recycle-bin') return 'recycle_bin';
    return 'dashboard';
  }, [location.pathname]);
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-warm-50 dark:bg-warm-900">
      ...
      {/* Sidebar - Desktop */}
      <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-warm-50 dark:bg-warm-800 border-r border-warm-200 dark:border-warm-700 transform transition-transform duration-300 lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-full flex flex-col">
          <div className="h-16 flex items-center px-6 border-b border-warm-200 dark:border-warm-700">
            <CheckSquare className="w-8 h-8 text-primary-600 mr-2" />
            <span className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary-500 to-primary-700">TaskMaster</span>
          </div>

          <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
            <button
              onClick={() => { navigateTo('dashboard'); setIsSidebarOpen(false); }}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${currentView === 'dashboard' ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'text-warm-600 dark:text-warm-400 hover:bg-warm-100 dark:hover:bg-warm-700'}`}
            >
              <LayoutDashboard size={20} />
              Dashboard
            </button>
            <button
              onClick={() => { navigateTo('tasks'); setIsSidebarOpen(false); }}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${currentView === 'tasks' ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'text-warm-600 dark:text-warm-400 hover:bg-warm-100 dark:hover:bg-warm-700'}`}
            >
            ...
            <button
              onClick={() => { navigateTo('calendar'); setIsSidebarOpen(false); }}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${currentView === 'calendar' ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'text-warm-600 dark:text-warm-400 hover:bg-warm-100 dark:hover:bg-warm-700'}`}
            >
            ...
            <button
              onClick={() => { navigateTo('archived'); setIsSidebarOpen(false); }}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${currentView === 'archived' ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'text-warm-600 dark:text-warm-400 hover:bg-warm-100 dark:hover:bg-warm-700'}`}
            >
              <Archive size={20} />
              Archived
            </button>
            <button
              onClick={() => { navigateTo('recycle_bin'); setIsSidebarOpen(false); }}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${currentView === 'recycle_bin' ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'text-warm-600 dark:text-warm-400 hover:bg-warm-100 dark:hover:bg-warm-700'}`}
            >
              <Trash2 size={20} />
              Recycle Bin
            </button>

            <div className="pt-4 mt-4 border-t border-warm-100 dark:border-warm-700">
              <p className="px-4 text-xs font-semibold text-warm-400 uppercase tracking-wider mb-2">Data Management</p>

              {/* JSON Export */}
              <button
                onClick={handleExportJSON}
                className="w-full flex items-center gap-3 px-4 py-2 text-sm text-warm-600 dark:text-warm-400 hover:bg-warm-100 dark:hover:bg-warm-700 rounded-lg"
              >
              ...
              <div className="relative">
                ...
                />
                <button
                  className="w-full flex items-center gap-3 px-4 py-2 text-sm text-warm-600 dark:text-warm-400 hover:bg-warm-100 dark:hover:bg-warm-700 rounded-lg pointer-events-none"
                >
          ...
          <div className="p-4 border-t border-warm-200 dark:border-warm-700">
            ...
            <div className="flex items-center gap-3 px-2">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-warm-900 dark:text-warm-50 truncate">{user.name}</p>
      ...
      <main className="flex-1 lg:ml-64 flex flex-col h-screen overflow-hidden">
        {/* Header */}
        <header className="h-16 bg-warm-50 dark:bg-warm-800 border-b border-warm-200 dark:border-warm-700 flex items-center justify-between px-4 lg:px-8">
          <div className="flex items-center gap-4">
            <button
              onClick={() => setIsSidebarOpen(true)}
              className="lg:hidden p-2 text-warm-600 dark:text-warm-300 hover:bg-warm-100 dark:hover:bg-warm-700 rounded-lg"
            >
              <Menu size={20} />
            </button>
            <h2 className="text-xl font-semibold text-warm-800 dark:text-warm-50 capitalize">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              <button
                ...
                className="p-2 rounded-full hover:bg-warm-100 dark:hover:bg-warm-700 text-warm-600 dark:text-warm-300 relative"
              >
                <Bell size={20} />
              ...
              {isNotificationsOpen && (
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-80 bg-warm-50 dark:bg-warm-800 rounded-xl shadow-xl border border-warm-100 dark:border-warm-700 z-50 animate-fadeIn overflow-hidden">
                    <div className="px-4 py-3 border-b border-warm-100 dark:border-warm-700 bg-warm-50 dark:bg-warm-700/50">
                      <h3 className="font-semibold text-warm-800 dark:text-warm-50 text-sm">Notifications</h3>
                    </div>
                    <div className="max-h-80 overflow-y-auto">
                      {notifications.length === 0 ? (
                        <div className="p-4 text-center text-sm text-warm-500">No new notifications</div>
                      ) : (
                        notifications.map(notif => (
                          <div
                            key={notif.id}
                            className="px-4 py-3 border-b border-warm-100 dark:border-warm-700 hover:bg-warm-50 dark:hover:bg-warm-700 transition-colors cursor-pointer"
                            onClick={() => {
                          ...
                          >
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                <p className="text-sm text-warm-800 dark:text-warm-200">{notif.message}</p>
                                <span className="text-xs text-warm-400 mt-1 block">{formatTime(notif.timestamp)}</span>
                              </div>
                            </div>
                          </div>
                        ))
            ...
            <div className="hidden md:flex items-center text-sm text-warm-500 dark:text-warm-400 border-l border-warm-200 dark:border-warm-700 pl-4">
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-8">
          ...
          {currentView === 'calendar' ? (
            <div className="max-w-7xl mx-auto space-y-6 animate-fadeIn h-full flex flex-col">
              <div className="bg-warm-50 dark:bg-warm-800 rounded-xl shadow-sm border border-warm-100 dark:border-warm-700 flex-1 flex flex-col overflow-hidden">
                {/* Calendar Header */}
                <div className="p-4 flex items-center justify-between border-b border-warm-100 dark:border-warm-700">
                  <h2 className="text-xl font-bold text-warm-900 dark:text-warm-50">
                    {currentDate.toLocaleString('default', { month: 'long', year: 'numeric' })}
                  </h2>
                  <div className="flex gap-2">
                    <button onClick={handlePrevMonth} className="p-2 hover:bg-warm-100 dark:hover:bg-warm-700 rounded-lg">
                      <ChevronLeft size={20} className="text-warm-600 dark:text-warm-300" />
                    </button>
                    ...
                    <button onClick={handleNextMonth} className="p-2 hover:bg-warm-100 dark:hover:bg-warm-700 rounded-lg">
                      <ChevronRight size={20} className="text-warm-600 dark:text-warm-300" />
                    </button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-7 border-b border-warm-100 dark:border-warm-700">
                  {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
                    <div key={day} className="p-3 text-center text-sm font-semibold text-warm-500 dark:text-warm-400 border-r border-warm-100 dark:border-warm-700 last:border-r-0">
                      {day}
                    </div>
                  ))}
                </div>
                ...
                <div className="flex-1 overflow-y-auto">
                  <div className="grid grid-cols-7 auto-rows-fr">
                    ...
                    {Array.from({ length: getFirstDayOfMonth(currentDate) }).map((_, i) => (
                      <div key={`empty-${i}`} className="min-h-[8rem] bg-warm-50/50 dark:bg-warm-900/30 border-b border-r border-warm-100 dark:border-warm-700 last:border-r-0"></div>
                    ))}
                    ...
                    {Array.from({ length: getDaysInMonth(currentDate) }).map((_, i) => {
                      ...
                      return (
                        <div
                          key={day}
                          className={`min-h-[8rem] p-2 border-b border-r border-warm-100 dark:border-warm-700 last:border-r-0 hover:bg-warm-50 dark:hover:bg-warm-700/50 transition-colors cursor-pointer group relative`}
                          onClick={() => handleDateClick(day)}
                          onDragOver={handleDragOver}
                          onDrop={(e) => handleDrop(e, day)}
                        >
                          <div className={`text-sm font-medium w-7 h-7 flex items-center justify-center rounded-full mb-1 ${isToday ? 'bg-primary-600 text-white shadow-md' : 'text-warm-700 dark:text-warm-300'}`}>
                            {day}
                          </div>
                            ...
                            {dayTasks.map(t => {
                              ...
                              return (
                                <div
                                  ...
                                  className={`text-xs px-1.5 py-1 rounded truncate border-l-2 cursor-grab active:cursor-grabbing hover:opacity-80 shadow-sm ${t.status === 'Completed' ? 'bg-warm-100 text-warm-500 border-warm-400 dark:bg-warm-700 dark:text-warm-400 line-through' :
                          ...
                          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 pointer-events-none">
                            <Plus className="text-warm-300 dark:text-warm-600 w-8 h-8" />
                          </div>
                        </div>
                      );
                    })}
            ...
            <div className="space-y-6 max-w-7xl mx-auto animate-fadeIn">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                  ...
                  <div key={i} className="bg-warm-50 dark:bg-warm-800 p-6 rounded-xl shadow-sm border border-warm-100 dark:border-warm-700 flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-warm-500 dark:text-warm-400">{stat.label}</p>
                      <p className="text-3xl font-bold text-warm-900 dark:text-warm-50 mt-1">{stat.value}</p>
                    </div>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div className="xl:col-span-2 space-y-6">
                  ...
                  <div className="bg-warm-50 dark:bg-warm-800 p-6 rounded-xl shadow-sm border border-warm-100 dark:border-warm-700">
                    <h3 className="text-lg font-bold text-warm-900 dark:text-warm-50 mb-4">Task Priority Distribution</h3>
                      ...
                      <div className="w-full md:w-1/2 h-full">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            ...
                            <Tooltip
                              contentStyle={{ backgroundColor: darkMode ? '#3d352e' : '#fff', borderColor: darkMode ? '#574c42' : '#e4ddd4', color: darkMode ? '#fff' : '#000' }}
                            />
                          </PieChart>
                        </ResponsiveContainer>
                      </div>
                          ...
                          return (
                            <div key={item.name} className="flex items-center justify-between group">
                              <div className="flex items-center gap-3">
                                ...
                                <span className="text-warm-600 dark:text-warm-300 font-medium">{item.name} Priority</span>
                              </div>
                              <div className="flex items-center gap-4">
                                <span className="text-warm-900 dark:text-warm-50 font-bold">{item.value}</span>
                                <span className="text-xs text-warm-400 w-10 text-right">{percentage}%</span>
                              </div>
                            </div>
                          )
                        })}
                        {priorityData.every(i => i.value === 0) && (
                          <p className="text-sm text-warm-400 text-center italic">No pending tasks to display</p>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-warm-50 dark:bg-warm-800 p-6 rounded-xl shadow-sm border border-warm-100 dark:border-warm-700">
                    <h3 className="text-lg font-bold text-warm-900 dark:text-warm-50 mb-6">30 Days Activity</h3>
                    <div className="h-64">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={monthlyActivityData}>
                          <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={darkMode ? '#574c42' : '#e4ddd4'} />
                          <XAxis dataKey="name" stroke={darkMode ? '#a89a8a' : '#8a7a68'} interval={0} tick={{ fontSize: 10 }} />
                          <YAxis stroke={darkMode ? '#a89a8a' : '#8a7a68'} />
                          <Tooltip
                            contentStyle={{ backgroundColor: darkMode ? '#3d352e' : '#fff', borderColor: darkMode ? '#574c42' : '#e4ddd4', color: darkMode ? '#fff' : '#000' }}
                          />
                          <Legend />
                ...
                <div className="bg-warm-50 dark:bg-warm-800 p-6 rounded-xl shadow-sm border border-warm-100 dark:border-warm-700 flex flex-col h-full max-h-[800px]">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-lg font-bold text-warm-900 dark:text-warm-50 flex items-center gap-2">
                      <History size={20} className="text-primary-500" /> Recent Activity
                    </h3>
                    <span className="text-xs text-warm-400">{activities.length} records</span>
                  </div>

                  <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
                    {activities.length === 0 ? (
                      <div className="text-center text-warm-400 py-10">No recent activity</div>
                    ) : (
                      activities.map(log => (
                        <div key={log.id} className="flex gap-3 items-start border-b border-warm-100 dark:border-warm-700 pb-3 last:border-0">
                          <div className={`mt-1 p-1.5 rounded-full shrink-0 ${log.action === 'Created' ? 'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' :
                          ...
                          <div>
                            ...
                            <div className="flex gap-2 text-xs text-warm-400 mt-1">
            ...
            <div className="max-w-7xl mx-auto space-y-6 animate-fadeIn h-full flex flex-col">
              {/* Controls Toolbar */}
              <div className="flex flex-col xl:flex-row gap-4 justify-between items-start xl:items-center bg-warm-50 dark:bg-warm-800 p-4 rounded-xl shadow-sm border border-warm-100 dark:border-warm-700 shrink-0">
                <div className="flex flex-col md:flex-row gap-3 w-full xl:w-auto">
                  <div className="relative group">
                    ...
                    <input
                      ...
                      className="pl-10 pr-4 py-2 w-full md:w-64 rounded-lg bg-warm-50 dark:bg-warm-700 border border-warm-200 dark:border-warm-600 focus:ring-2 focus:ring-primary-500 outline-none text-sm dark:text-warm-50"
                    />
                  </div>
                  <div className="flex gap-2 overflow-x-auto pb-1 md:pb-0">
                    <select
                      ...
                      className="px-3 py-2 rounded-lg bg-warm-50 dark:bg-warm-700 border border-warm-200 dark:border-warm-600 text-sm text-warm-700 dark:text-warm-200 outline-none"
                    >
                ...
                <div className="flex gap-3 w-full xl:w-auto justify-between xl:justify-end">
                  <div className="flex gap-1 bg-warm-100 dark:bg-warm-700 p-1 rounded-lg">
                    <button
                      onClick={() => setViewMode('grid')}
                      title="Grid View"
                      className={`p-2 rounded-md transition-all ${viewMode === 'grid' ? 'bg-warm-50 dark:bg-warm-600 shadow-sm text-primary-600 dark:text-primary-400' : 'text-warm-500 dark:text-warm-400'}`}
                    >
                      <Grid size={18} />
                    </button>
                    <button
                      ...
                      title="List View"
                      className={`p-2 rounded-md transition-all ${viewMode === 'list' ? 'bg-warm-50 dark:bg-warm-600 shadow-sm text-primary-600 dark:text-primary-400' : 'text-warm-500 dark:text-warm-400'}`}
                    >
                      <ListIcon size={18} />
                    </button>
                    <button
                      ...
                      title="Kanban Board"
                      className={`p-2 rounded-md transition-all ${viewMode === 'board' ? 'bg-warm-50 dark:bg-warm-600 shadow-sm text-primary-600 dark:text-primary-400' : 'text-warm-500 dark:text-warm-400'}`}
                    >
                      <Columns size={18} />
                    </button>
                  </div>
              ...
              {filteredTasks.length === 0 ? (
                <div className="text-center py-20 bg-warm-50 dark:bg-warm-800 rounded-xl border border-dashed border-warm-300 dark:border-warm-700">
                  <div className="mx-auto w-16 h-16 bg-warm-100 dark:bg-warm-700 rounded-full flex items-center justify-center mb-4">
                    {currentView === 'recycle_bin' ? <Trash2 className="w-8 h-8 text-warm-400" /> : <CheckSquare className="w-8 h-8 text-warm-400" />}
                  </div>
                  ...
                  {viewMode === 'board' && (
                    <div className="flex-1 overflow-x-auto overflow-y-hidden pb-4">
                      <div className="flex gap-6 h-full min-w-[1000px]">
                        ...
                        <div className="flex-1 flex flex-col bg-warm-100 dark:bg-warm-800/50 rounded-xl p-4 h-full overflow-hidden">
                          <div className="flex items-center justify-between mb-4 px-2">
                            <h3 className="font-semibold text-warm-700 dark:text-warm-300 flex items-center gap-2">
                              <div className="w-3 h-3 rounded-full bg-blue-500"></div> {currentView === 'recycle_bin' ? 'Deleted' : 'To Do'}
                            </h3>
                            <span className="text-xs bg-warm-50 dark:bg-warm-700 px-2 py-1 rounded-full text-warm-500">{filteredTasks.filter(t => t.status === 'Pending').length}</span>
                          </div>
                          <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
                        ...
                        <div className="flex-1 flex flex-col bg-warm-100 dark:bg-warm-800/50 rounded-xl p-4 h-full overflow-hidden">
                          <div className="flex items-center justify-between mb-4 px-2">
                            ...
                            <span className="text-xs bg-warm-50 dark:bg-warm-700 px-2 py-1 rounded-full text-warm-500">{filteredTasks.filter(t => t.status === 'Completed').length}</span>
                          </div>
                          <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
        ...
        <form onSubmit={handleSaveTask} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 rounded-lg border border-warm-300 dark:border-warm-600 bg-warm-50 dark:bg-warm-700 dark:text-warm-50 focus:ring-2 focus:ring-primary-500 outline-none"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-warm-700 dark:text-warm-300 mb-1">Priority</label>
              <select
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-warm-700 dark:text-warm-300 mb-1">Start Date</label>
              <input
                required
                type="datetime-local"
            ...
            <div>
              <label className="block text-sm font-medium text-warm-700 dark:text-warm-300 mb-1">Deadline (End)</label>
              <input
                required
                type="datetime-local"
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3 py-2 rounded-lg border border-warm-300 dark:border-warm-600 bg-warm-50 dark:bg-warm-700 dark:text-warm-50 focus:ring-2 focus:ring-primary-500 outline-none resize-none"
              placeholder="Add details..."
            />
          </div>

          <div>
            <label className="block text-sm font-medium text-warm-700 dark:text-warm-300 mb-1">Subtasks (Checklist)</label>
            <div className="flex gap-2 mb-2">
              <input
                ...
                className="flex-1 px-3 py-2 rounded-lg border border-warm-300 dark:border-warm-600 bg-warm-50 dark:bg-warm-700 dark:text-warm-50 focus:ring-2 focus:ring-primary-500 outline-none text-sm"
              />
              <button type="button" onClick={handleAddSubtask} className="px-3 py-2 bg-warm-100 dark:bg-warm-600 rounded-lg hover:bg-warm-200 dark:hover:bg-slate-500">
                <Plus size={18} />
              </button>
            </div>
            <div className="space-y-2 max-h-32 overflow-y-auto custom-scrollbar">
              {newTaskForm.subtasks?.map(st => (
                <div key={st.id} className="flex items-center justify-between bg-warm-50 dark:bg-warm-700/50 px-3 py-2 rounded-lg">
                  <span className="text-sm text-warm-700 dark:text-warm-300 truncate">{st.title}</span>
          ...
          <div>
            <label className="block text-sm font-medium text-warm-700 dark:text-warm-300 mb-1">Image URL (Optional)</label>
            <div className="flex gap-2">
              <input
                type="url"
              ...
              {newTaskForm.imageUrl && (
                <div className="w-10 h-10 rounded overflow-hidden border border-warm-200 shrink-0">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-warm-100 dark:border-warm-700">
            <button
              type="button"
              onClick={closeTaskModal}
              className="px-4 py-2 text-warm-700 dark:text-warm-300 hover:bg-warm-100 dark:hover:bg-warm-700 rounded-lg transition-colors"
            >
              Cancel
            </button>
            <button
...
const LoginPage: React.FC<{
  ...
  addToast: (message: string, type?: Toast['type']) => void;
  removeToast: (id: string) => void;
}> = ({ setUser, darkMode, setDarkMode, toasts, addToast, removeToast }) => {
  const navigate = useNavigate();
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-warm-100 dark:bg-warm-900 px-4">
      <ToastContainer toasts={toasts} removeToast={removeToast} />
      <div className="max-w-md w-full bg-warm-50 dark:bg-warm-800 rounded-2xl shadow-xl p-8 transform transition-all">
        <div className="text-center mb-8">
          <div className="mx-auto w-16 h-16 bg-primary-100 dark:bg-primary-900/30 rounded-full flex items-center justify-center mb-4">
            <CheckSquare className="w-8 h-8 text-primary-600 dark:text-primary-400" />
          </div>
          <h1 className="text-3xl font-bold text-warm-900 dark:text-warm-50">Welcome Back</h1>
          <p className="text-warm-500 dark:text-warm-400 mt-2">Sign in to manage your tasks</p>
        </div>

        <form onSubmit={handleLogin} className="space-y-6">
          <div>
            <label className="block text-sm font-medium text-warm-700 dark:text-warm-300 mb-1">Username</label>
            <input
              type="text"
              value={usernameInput}
              onChange={(e) => setUsernameInput(e.target.value)}
              className="w-full px-4 py-3 rounded-lg border border-warm-300 dark:border-warm-600 focus:ring-2 focus:ring-primary-500 focus:border-transparent bg-warm-50 dark:bg-warm-700 dark:text-warm-50 outline-none transition-all"
              placeholder="admin"
            />
          </div>
          <div>
...
const App: React.FC = () => {
  ...
  return (
    <BrowserRouter>
      <Routes>
        <Route path="/" element={<LandingPage user={user} darkMode={darkMode} setDarkMode={setDarkMode} />} />
        <Route path="/login" element={<LoginPage setUser={setUser} darkMode={darkMode} setDarkMode={setDarkMode} toasts={toasts} addToast={addToast} removeToast={removeToast} />} />
        <Route path="/dashboard" element={
          <ProtectedRoute user={user}>
            <DashboardApp user={user!} setUser={setUser} darkMode={darkMode} setDarkMode={setDarkMode} toasts={toasts} addToast={addToast} removeToast={removeToast} />
          </ProtectedRoute>
        } />
